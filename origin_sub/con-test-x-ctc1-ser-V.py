# -*- coding: utf-8 -*-
"""
Created on Mon Aug 26 08:21:24 2024
...
op='01'
l=os.listdir('../Fluo-N3DH-CE/test/'+op+'/')
...
# EN.load_state_dict(torch.load('../model/EN+cer2-149.0-

U.load_state_dict(torch.load('./xmodel/U-ext+sc2n-199.0-8.0351.pth'))
EX.load_state_dict(torch.load('./xmodel/EX+sc2n-199.0-8.0351.pth'))
EN.load_state_dict(torch.load('./xmodel/EN+sc2n-199.0-8.0351.pth'))


# U.load_state_dict(torch.load('../xmodel/U-ext+ce9-8.0-0.0000.pth'))
# EX.load_state_dict(torch.load('../xmodel/EX+ce9-8.0-0.0000.pth'))
# EN.load_state_dict(torch.load('../xmodel/EN+ce9-8.0-0.0000.pth'))
weights = torch.tensor([1.0, 16.0]).cuda()
criterion = torch.nn.CrossEntropyLoss(reduction='none',weight=weights)
weights2 = torch.tensor([1.0, 16.0,16.0]).cuda()
...
#model=torch.load('D:/model/2db.pth')
win2=[100,150,8] 
win2=[120,90,9]
...
def ud(x):
    ...
    xn[xn<0]=0
    return x+xn
DDK7=torch.ones([7,7]).float().to(device).reshape([1,1,7,7,1])
DDK15=torch.ones([15,15]).float().to(device).reshape([1,1,15,15,1])
def vsup256(U,x,la):
    with torch.no_grad():
    ...
    uko=uk.copy()
    uc=uc.squeeze(1)
    #uu=label(uk*(uo.argmax(1).cpu().squeeze()>1).numpy())
    # uf=np.unique(uu[uc.cpu()[0]>0])
    # uf=uf[uf>0]
    # for i in regionprops(uu):
    #     #if i.label not in uf:
    #         x1,y1,z1,x2,y2,z2=i.bbox
    #         if (x2-x1>20 or y2-y1>20) or z2-z1>8 :
    #             uk[x1:x2,y1:y2,z1:z2][i.image]=cut(i.image.copy())[i.image]
...
def cut(x):
    ...
    for i in range(1,x1.shape[0]-1):
        if x1[i]+1<x1[i+1] and x1[i]+1<x1[i-1]:
            x[i]=0
        # if  x1[i]+1<=x1[i+1:i+3].mean() and 1+x1[i]<=x1[i-3:i].mean():
        #              x[i]=0
    for i in range(1,x2.shape[0]-1):
        if x2[i]+1<x2[i+1] and x2[i]+1<x2[i-1]:
            x[:,i]=0
        # if x2[i]+1<=x2[i+1:i+3].mean() and 1+x2[i]<=x2[i-3:i].mean():
        #     x[:,i]=0
    for i in range(1,x3.shape[0]-1):
...
for step, batch in enumerate(tk0):
    ...
    with torch.no_grad():
                if step==0:
                    ...
                    uu=(cp3(p1,nb.cuda())>0)#*(s1>3)#cp2(p1,BA.max(-1)[0].cuda())
                    p1=p1[uu]
                    f1=f1[uu]
                    zs1=zs1[uu]
                    s1=s1[uu]
...
#######################################################
        ...
        qx,q=distmat.topk(min(10,px.shape[0],py.shape[0]),largest=False)
            ...
            if len(rs)>0:
                ...
                if 1:
                                ...
                                for st in ss[1:]:
                                    ...
                                    if  nrt[st] not in zz and nres[st]<min(max(su,5)*4,80):
                                        rtx.append(nrt[st])
                                        rts.append(nrs[st])
                                        rtl.append(nres[st])
                                        zz=zz+dx2[nrt[st]]
                                ...
                                r1s[di]=rts[:4]
                                r1l[di]=rtl[:4]
                                if r1f.get(di,0)<np.max(g):r1f[di]=np.max(g)
                                if r1f.get(di,0)==0:r1f[di]=0
                                r1su[di]=su
    #if step==106:asfsaf
    for i in r:
                       
                          #r[i]=[r[i][x] for x in range(min(len(r[i]),1+int(r1f[i]>0.1 or (r1s[i][-1]>0.1 and r1f[i]>0.01  )))) if  ((x==0 and r1s[i][0]>0.1) or (x>0))]
                          #r[i]=[[r[i][x],r1s[i][x]] for x in range(min(len(r[i]),1+int(r1f[i]>0.001) * ((np.array(r1s[i])>0.1).sum()-1 )  )) if  ((x==0 and r1s[i][0]>0.1) or (x>0))]
            #rdr[i]=[[r[i][x],r1s[i][x]] for x in range(min(len(r[i]),1+int(r1f[i]>0.005) * ((np.array(r1s[i])>0.1).sum()-1 )  )) if  ((x==0 and r1s[i][0]>0.1) or (x>0))]
    #         r[i]=[[r[i][x],r1s[i][x]] for x in range(min(len(r[i]),1+int(r1f[i]>0.01) * ((np.array(r1s[i])>0.1).sum()-1 )  )) if  ((x==0 and r1s[i][0]>0.1) or (x>0))]
        # if  step==188:
        #      r[i]=[[r[i][x],r1s[i][x]] for x in range(len(r[i])) if (x==0 and r1s[i][x]>0.1)  or (r1f[i]>0.01 and r1s[i][x]>0.1)]
        # else:
             r[i]=[[r[i][x],r1s[i][x]] for x in range(len(r[i])) if (x==0 and r1s[i][x]>0.1)  or (r1f[i]>0.01 and r1s[i][x]>0.1) or ( r1f[i]>0.005 and r1s[i][x]>0.2 )]
        ...
        for j in r.copy():
            ...
            if j not in mid and len(r[j])>0:
                ...
                if np.sum(u)==0:
                    ...
                    if zaq[j][0].item() in midn and len(r.get(zaq[j][0].item(),[]))>0 and tu[0]==np.max(tu) and j not in dx.get(zaq[j][0].item() ,[]):
                    ...
                    elif zaq[j][1].item() in midn and len(r.get(zaq[j][1].item(),[]))>0 and  j not in dx.get(zaq[j][1].item() ,[]):
                        ra=zaq[j][1].item()
                        if ra not in jr:continue
                        fb=py1[r[j]]
                        fa=py1[r[ra]]
                        ...
                        if flag2:
                            ...
                            nid=nid+2
                            ff=2
                    elif zaq[j][2].item() in midn and len(r.get(zaq[j][2].item(),[]))>10 and  j not in dx.get(zaq[j][2].item() ,[]):
                        ra=zaq[j][2].item()
                        if ra not in jr:continue
                        fb=py1[r[j]]
                        fa=py1[r[ra]]
                        ...
                        ttla=(torch.sqrt((txa-tx0)*(txa-tx0)+(tya-ty0)*(tya-ty0)+0*(tza-tz0)*(tza-tz0)))
                        ttla2=(torch.sqrt((txa2-tx0)*(txa2-tx0)+(tya2-ty0)*(tya2-ty0)+(tza2-tz0)*(tza2-tz0)))
                        flag2=(ttla<=ttl1 and ttla<=ttl2 and ttla<0 and torch.abs(ttl1-ttl2)<30) or ( ttl3>=ttl1 and ttl4>=ttl2 and  ttla<30)
                        if flag2:
                    ...
                    if ff>0:
                        ...
                        q1=fill(q1,jj[0],jj[1],jj[2]//10,v=1,s=min(50,max(10,s1[j].item())//1))
                        ...
                        q1=fill(q1,jj1[0],jj1[1],jj1[2]//10,v=1,s=1)
                        q=(pic==midn[ra])+q1
                        pic[q>0]=mid[ra]
        ...
        if j in r:
            ...
            if len(dj)==1 :   
                ...
                if 1:#ta in ct:
                    q1=np.zeros(pic.shape)
                    q2=np.zeros(pic.shape)
                    q1=fill(q1,jj[0],jj[1],jj[2]//10,v=1,s=min(50,max(10,s2[dj[0]].item())//1))
                    q2=fill(q2,jj[0],jj[1],jj[2]//10,v=1,s=3)
                    tt=cx==ta
                    q1=q1*tt*(pic==0)+q2
                    pic[q1>0]=mid[j]
                    #if 185 in pic:adad
                ...
                tas=[cx[py[qi][0],py[qi][1],py[qi][2]//10] for qi in ui]
                for qi in ui:
                    jj=py[qi]
                    ta=cx[jj[0],jj[1],jj[2]//10]
                    ntl+=dx2ex[qi]
    ...
    cv2.imwrite('./ctc1/'+uk+'.png',np.concatenate([a,b,c],1)) 
    #if step==26:dsfsdf
...
path='../Fluo-N3DH-CE/01_RES/'
pa=np.sort(os.listdir('../Fluo-N3DH-CE/01_RES/'))
S={}
E={}
B={}
now=[]
...
for i in nkd:
    ...
    if len(nkd[i])>0:
        ...
        if len(nkd[i])>2:
            ...
            ff=1
        for j in nkd[i].copy():
            if ns.get(j,0)<189 and nl.get(j,-1)<=3 and (ne.get(j,0)<189 and len(nkd.get(j,[]))<1):
               
                nkd[i].remove(j)
                fno.append(j)
            elif  (ne.get(j,0)<40 and len(nkd.get(j,[]))<1):
...
for i in tqdm(range(len(Kim))):
    if 1:
        ...
        b=tq1
        c=tq2
        cv2.imwrite('./ctc1x/'+uk+'.png',np.concatenate([a,b,c],1)) 
        #if i==15:asdas
for j in now:
    E[j]=189 
fkd={}
...
rd.to_csv('../Fluo-N3DH-CE/01_RES/res_track.txt',index=False,header=False,sep=' ')
