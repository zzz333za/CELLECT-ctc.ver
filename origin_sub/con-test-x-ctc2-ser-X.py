...
# U.load_state_dict(torch.load('../xmodel/U-ext+sc2n-73.0-11.6475.pth'))
# EX.load_state_dict(torch.load('../xmodel/EX+sc2n-73.0-11.6475.pth'))
# EN.load_state_dict(torch.load('../xmodel/EN+sc2n-73.0-11.6475.pth'))
# U.load_state_dict(torch.load('X:/xmodel/U-ext+sc2n27-119.0-2.7188.pth'))
# EX.load_state_dict(torch.load('X:/xmodel/EX+sc2n27-119.0-2.7188.pth'))
# EN.load_state_dict(torch.load('X:/xmodel/EN+sc2n27-119.0-2.7188.pth'))
# a='sc242-71.0-5.4584'
# U.load_state_dict(torch.load('X:/xmodel/U-ext+'+a+'.pth'))
# EX.load_state_dict(torch.load('X:/xmodel/EX+'+a+'.pth'))
# EN.load_state_dict(torch.load('X:/xmodel/EN+'+a+'.pth'))

# a='sc2n-40.0'
# U.load_state_dict(torch.load('C:/Users/z/Desktop/ctc-for pub/src/CTC submission/Trained models/U-ext+'+a+'.pth'))
# EX.load_state_dict(torch.load('C:/Users/z/Desktop/ctc-for pub/src/CTC submission/Trained models/EX+'+a+'.pth'))
# EN.load_state_dict(torch.load('C:/Users/z/Desktop/ctc-for pub/src/CTC submission/Trained models/EN+'+a+'.pth'))
...
w=torch.ones(1,1, 39,39,39)
nb=torch.nn.functional.conv3d(BA.unsqueeze(0), w, stride=1, padding=(19,19,19)).squeeze()
T=[]
...
for step, batch in enumerate(tk0):
    ...
    with torch.no_grad():
                if step==0:
                    ...
                    p1,f1,zs1,s1,lb1,uo =  bvsup256(U,(PA(torch.cat([in2,inputs],1))),labels,xz1=x1,xz2=x2,yz1=y1,yz2=y2)
...
#######################################################
    ...
    for i in r:
            ...
            #r[i]=[[r[i][x],r1s[i][x]] for x in range(min(len(r[i]),1+int(r1f[i]>0.005) * ((np.array(r1s[i])>0.1).sum()-1 )  )) if  ((x==0 and r1s[i][0]>0.1) or (x>0))]
             # if step==138:
...
path='../Fluo-N3DH-CE/02_RES/'
pa=np.sort(os.listdir('../Fluo-N3DH-CE/02_RES/'))
S={}
E={}
B={}
now=[]
...
for i in nkd:
    ...
    if len(nkd[i])>=0:
        ...
        if len(nkd[i])>2:
            ...
            ff=1
        for j in nkd[i].copy():
            if ns.get(j,0)<135 and nl.get(j,-1)<=0 and (ne.get(j,0)<139 and len(nkd.get(j,[]))<1):
...
rd.to_csv('../Fluo-N3DH-CE/02_RES/res_track.txt',index=False,header=False,sep=' ')
