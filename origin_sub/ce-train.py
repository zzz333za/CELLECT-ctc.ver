...
def tim(a):
    ...
    return b
op='01'
l=os.listdir('../Fluo-N3DH-CE/train/'+op+'/')
...
for i in tqdm(l):
    if 'tif' in i:         
        ...
        num=int(i.split('.')[0][1:])
        D[num]='../Fluo-N3DH-CE/train/'+op+'/'+i
        


d1=pd.read_table('../Fluo-N3DH-CE/train/'+op+'_GT/TRA/man_track.txt',sep=' ',header=None).values#,skiprows=3)
gd1={}
gj1={}
for i in d1:
...
for i in tqdm(l):
    if 'tif' in i:         
        ...
        num=int(i.split('.')[0][1:])
        vD[num]='../Fluo-N3DH-CE/train/'+op+'/'+i
d1=pd.read_table('../Fluo-N3DH-CE/train/'+op+'_GT/TRA/man_track.txt',sep=' ',header=None).values#,skiprows=3)
gd2={}
gj2={}
for i in d1:
...
for i in tqdm(range(200)):
    if i<=192+2:
        ...
        K3[D[i]]=rget(tim(Image.open('../Fluo-N3DH-CE/train/01_ST/SEG/man_seg'+D[i].split('/')[-1][1:])))
        K1[D[i]]=tim(Image.open('../Fluo-N3DH-CE/train/01_GT/TRA/man_track'+D[i].split('/')[-1][1:]))
        K2[D[i]]=tim(Image.open('../Fluo-N3DH-CE/train/01_ST/SEG/man_seg'+D[i].split('/')[-1][1:]))
    if i <=187+2:    
        KK[vD[i]]=tim(Image.open(vD[i]))
        K1[vD[i]]=tim(Image.open('../Fluo-N3DH-CE/train/02_GT/TRA/man_track'+vD[i].split('/')[-1][1:]))
        K2[vD[i]]=tim(Image.open('../Fluo-N3DH-CE/train/02_ST/SEG/man_seg'+vD[i].split('/')[-1][1:]))
        
        K3[vD[i]]=rget(tim(Image.open('../Fluo-N3DH-CE/train/02_ST/SEG/man_seg'+vD[i].split('/')[-1][1:])))
...
class IntracranialDataset(Dataset):
    ...
    def __len__(self):
        ...
        return 400#len(self.data)+185-3
    ...
    def __getitem__(self, i):
        ...
        if i<=192:
            ...
            gj=gj1.get(j+o,[])
            o='01'
        elif i<=192+187 and i not in [192+176,192+177,192+178,192+179,192+180]:
            j=j-192
            g=vD[j]
            ...
            if o=='01':
             j=random.choice(list(range(170,192)))
             g=D[j]
             ...
             gj=gj1.get(j+1,[]) 
                ...
                j=random.choice(list(range(150,187)))
                g=vD[j]
        ...
        if random.randint(0,2)==1:
            noise=np.random.normal(loc=0,scale=random.randint(5,100),size=img.shape)
            noise2=np.random.normal(loc=0,scale=random.randint(5,100),size=img.shape)
            noise3=np.random.normal(loc=0,scale=random.randint(5,100),size=img.shape)
 
            img=(img+noise).clip(0,255)
            img3=(img3+noise2).clip(0,255)
            img2=(img2+noise3).clip(0,255)             
        ...
        xa,xb,xc=random.randint(30,256-30 ),random.randint(30,t.shape[1]-256-30 ),random.randint(0,max(0,t.shape[2]-31) )
...
class VDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        t=tim(Image.open('../Fluo-N3DH-CE/train/'+o+'_GT/TRA/man_track'+g.split('/')[-1][1:]))
        t6=tim(Image.open('../Fluo-N3DH-CE/train/'+o+'_ST/SEG/man_seg'+g.split('/')[-1][1:]))
        t16=tim(Image.open('../Fluo-N3DH-CE/train/'+o+'_ST/SEG/man_seg'+g2.split('/')[-1][1:]))
        t15=tim(Image.open('../Fluo-N3DH-CE/train/'+o+'_GT/TRA/man_track'+g2.split('/')[-1][1:]))
        t=cen(t)
        t15=cen(t15)
        t1=t15.copy()
        t7=t16.copy()
...
######################################################
    ...
    if epoch>=1 :
        win2s=[[128,256,0],[256,380,0],[256,0,0]]
        ...
        for step, batch in enumerate(tk0):
            ...
            for iix in range(3):
                ...
                in2=batch["im2"].unsqueeze(1)
                la2=batch["la2"]
                inputs = inputs[:,:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30]#.to(device, dtype=torch.float)
                labels = labels[:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30].to(device, dtype=torch.float)
                in2=in2[:,:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30]#.to(device, dtype=torch.float)
                la2=la2[:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30].to(device, dtype=torch.float)
        
                p1=batch["p1"].unsqueeze(1)[:,:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30].to(device, dtype=torch.float)
                p2=batch["p2"].unsqueeze(1)[:,:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30].to(device, dtype=torch.float)       
                s1=batch["s1"][:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30].to(device, dtype=torch.float)   
                s2=batch["s2"][:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30].to(device, dtype=torch.float)   
                lsize1=batch["size1"][:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30]#.to(device, dtype=torch.float)   
                ...
                in3= batch["img3"].unsqueeze(1)[:,:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30]#.to(device, dtype=torch.float)
                #in4= batch["img4"].unsqueeze(1)[:,:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+32].to(device, dtype=torch.float)
                # pl1=batch["p1"].unsqueeze(1)[:,:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30].to(device, dtype=torch.float)
                # pl2=batch["p2"].unsqueeze(1)[:,:,win2[0]:win2[0]+256,win2[1]:win2[1]+256,win2[2]:win2[2]+30].to(device, dtype=torch.float)       
        ...
        if  epoch >2:
            va=epoch_loss
            torch.save(EX.state_dict(),'./xmodel/EX+sc2n-{:.1f}.pth'.format(epoch))
            torch.save(EN.state_dict(),'./xmodel/EN+sc2n-{:.1f}.pth'.format(epoch))
            torch.save(U.state_dict(),'./xmodel/U-ext+sc2n-{:.1f}.pth'.format(epoch))
